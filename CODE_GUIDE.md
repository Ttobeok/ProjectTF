...
| `Clear` | 문 조준 + 2 | 한 명씩 차례로, 각자 자기 슬롯으로 진입하며 이동 중 사격. 방 안에서 3초간 적 없으면 "Room clear!" → Follow |
...
**navmesh는 모든 장애물을 에이전트 반경만큼 깎으므로, 문 근처 엄폐물이 문을 막을 수 있습니다.**
Cover_B1이 방 B 진입로에 걸쳐 있었는데, 35cm 깎이고 나니 문 벽 끝부분의 깎인 경계와 틈 없이
맞닿았습니다. 문은 플레이어에게는 열려 있고 AI에게는 닫혀 있었고, 두 방을 잇는 모든 경로가 측면
복도로 돌았는데, 로그가 전부 상태 전이라 아무것도 이상해 보이지 않았습니다. `-CQBTraceSquad`는
이것 때문에 만들었습니다 — 직선 800짜리 이동의 경로 길이가 3,040이면 그게 신호입니다.

**AI 컨트롤러는 자기 폰보다 오래 살지 않습니다.** 폰이 죽는 순간 빙의를 풀고 스스로를
파괴합니다. 그래서 **컨트롤러로 만든 목록은 전사자를 표시할 수 없습니다** — 다음 프레임에
그냥 사라집니다. HUD 명부가 폰 기준인 이유이고, 호출부호와 element를 빙의 시점에 폰으로
내려주는 이유입니다.

**대상을 참조한 채로 두면 시체가 상태머신을 잠급니다.** 시체가 영구히 남으므로, "대상이
죽으면 전투 종료" 규칙이 참조를 비우지 않으면 그 분기가 매 틱 참이 되어 아래의 전이가
영영 실행되지 않습니다. 교전에서 이긴 AI가 그대로 멈춰버립니다.
...
| `-CQBKillSide=<진영>` | `enemy`(기본) 또는 `ally`. HUD의 전사 표시를 검증할 때 씁니다 |
| `-CQBTraceSquad=<초>` | N초마다 분대원별 상태·위치·경로(지점 수, 길이, 목표)를 로그로 남깁니다 |
